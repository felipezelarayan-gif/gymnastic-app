  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-3 md:p-5 hover:border-zinc-700 hover:bg-zinc-800/70 transition">
      <div className="md:hidden flex items-center justify-between gap-2">
        <a href={`/alumnos/${alumno.id}`} className="flex items-center gap-3 min-w-0">
          <div className="h-10 w-10 rounded-full bg-emerald-500/10 border border-emerald-700 flex items-center justify-center text-sm font-bold text-emerald-400 shrink-0 overflow-hidden">
            {alumno.foto_url ? <img src={alumno.foto_url} alt={t("alumnos.fotoPerfil")} className="h-full w-full object-cover" /> : iniciales(alumno.nombre, alumno.apellido)}
          </div>
          <div className="min-w-0">
            <h3 className="font-semibold truncate text-sm">{nombreCompleto(alumno)}</h3>
            <p className="text-xs text-emerald-400 mt-0.5">{metricasLoading ? "..." : pendientes} {t("alumnos.pendientes")}</p>
      ...
      <div className="hidden md:flex items-center gap-4">
        <a href={`/alumnos/${alumno.id}`} className="flex items-center gap-4 min-w-0 flex-1 hover:opacity-80 transition">
          ...
          <div className="min-w-0">
            <h3 className="font-semibold truncate">{nombreCompleto(alumno)}</h3>
            <p className="text-xs text-zinc-400 mt-0.5">{t("alumnos.ultimo")}: {metricasLoading ? "..." : ultimoEntrenamiento}</p>
            <p className="text-xs text-zinc-500 mt-1.5">
              {alumno.email && <span>{alumno.email} · </span>}
              {metricasLoading ? "..." : `${finalizados} ${t("alumnos.finalizados")} · ${pendientes} ${t("alumnos.pendientes")}`}
            </p>
            <span className={`inline-block mt-1 px-2 py-0.5 rounded-full text-[10px] font-medium ${estado.colorClasses}`}>{estado.icono} {estado.label}</span>
          </div>
        </a>
