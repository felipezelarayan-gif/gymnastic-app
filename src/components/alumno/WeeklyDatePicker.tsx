...
export default function WeeklyDatePicker({
  ...
  const [semanaInicio, setSemanaInicio] = useState(() => {
    const hoy = new Date(selectedDate);
  ...
  function cambiarSemana(direccion: number) {
    ...
    nueva.setDate(semanaInicio.getDate() + direccion * 7);
  ...
  function hoy() {
    ...
    const dia = hoy.getDay();
    const diff = hoy.getDate() - dia + (dia === 0 ? -6 : 1);
    setSemanaInicio(new Date(hoy.setDate(diff)));
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold">
            {(idioma === "en" ? MESES_EN : MESES_ES)[semanaInicio.getMonth()]} {semanaInicio.getFullYear()}
          </span>
          {formatDateKey(semanaInicio) !== formatDateKey(new Date()) && (
