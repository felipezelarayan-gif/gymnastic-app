...
export default function AlumnoConfiguracionPage() {
  ...
  async function cambiarPassword() {
    ...
    if (!passwordActual || !passwordNueva || !passwordConfirmar) { mostrarToast("Completá todos los campos.", "error"); return; }
    if (passwordNueva !== passwordConfirmar) { mostrarToast("La nueva contraseña no coincide.", "error"); return; }
    if (passwordNueva.length < 8) { mostrarToast("La nueva contraseña debe tener al menos 8 caracteres.", "error"); return; }
    if (passwordActual === passwordNueva) { mostrarToast("La nueva contraseña debe ser diferente a la actual.", "error"); return; }
    setGuardandoPassword(true);
    try {
      const { data: { user }, error: userError } = await supabase.auth.getUser();
      if (userError || !user?.email) { mostrarToast("No se pudo validar la sesión.", "error"); router.push("/login"); return; }
      const { error: loginError } = await supabase.auth.signInWithPassword({ email: user.email, password: passwordActual });
      if (loginError) { mostrarToast("La contraseña actual no es correcta.", "error"); return; }
      ...
      setPasswordActual(""); setPasswordNueva(""); setPasswordConfirmar(""); setMostrarPassword(false);
    } finally { setGuardandoPassword(false); }
  }

  async function cerrarSesion() { await supabase.auth.signOut(); router.replace("/login"); }
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-white p-6 pb-24">
      <div className="max-w-4xl mx-auto">
        ...
        <SoporteCard
          remitenteId={alumno?.user_id || ""}
          remitenteNombre={`${alumno?.nombre || ""} ${alumno?.apellido || ""}`.trim()}
          remitenteEmail={alumno?.email || ""}
          remitenteRol="alumno"
