...
export default function SoporteMensajesPage() {
  ...
  async function cargarMensajes() {
    ...
    const { data: perfil } = await supabase.from("profiles").select("rol").eq("id", sessionData.session.user.id).maybeSingle();
    if (perfil?.rol !== "admin") { router.push("/soporte"); return; }
    ...
    <main className="min-h-screen bg-zinc-950 text-white p-6 pb-28">
      <div className="max-w-3xl mx-auto">
        ...
        <header className="mt-6 mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold">{t("soporte.mensajesPage.mensaje")}</h1>
            <p className="text-zinc-400 mt-2">
              {mensajes.filter((m) => !m.leido).length} {t("soporte.mensajesPage.noLeidos")} · {mensajes.length} {t("mensajes.totales")}
            </p>
          </div>
          <div className="flex gap-2">
            <button type="button" onClick={() => setFiltro("todos")} className={`rounded-lg px-3 py-1.5 text-xs font-medium border transition ${filtro === "todos" ? "border-emerald-600 bg-emerald-500/20 text-emerald-400" : "border-zinc-700 text-zinc-400 hover:bg-zinc-800"}`}>{t("soporte.mensajesPage.todos")}</button>
            <button type="button" onClick={() => setFiltro("no-leidos")} className={`rounded-lg px-3 py-1.5 text-xs font-medium border transition ${filtro === "no-leidos" ? "border-emerald-600 bg-emerald-500/20 text-emerald-400" : "border-zinc-700 text-zinc-400 hover:bg-zinc-800"}`}>{t("soporte.mensajesPage.noLeidos")}</button>
          </div>
        </header>

        {mensajesFiltrados.length === 0 ? (
          <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-8 text-center">
            <p className="text-zinc-400">{filtro === "no-leidos" ? t("soporte.mensajesPage.emptyNoLeidos") : t("soporte.mensajesPage.empty")}</p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold">{msg.remitente_nombre || t("soporte.mensajesPage.anonimo")}</span>
                    <span className="text-xs text-zinc-500">{msg.remitente_rol === "alumno" ? t("soporte.mensajesPage.alumno") : msg.remitente_rol === "profe" ? t("soporte.mensajesPage.profesor") : t("soporte.mensajesPage.soporte")}</span>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
            <div className="w-full max-w-lg bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-2xl">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-bold">{t("soporte.mensajesPage.mensaje")}</h3>
              ...
              <div className="space-y-3 text-sm">
                <div><span className="text-zinc-500">{t("soporte.mensajesPage.de")}</span> <span className="text-white">{mensajeSeleccionado.remitente_nombre || t("soporte.mensajesPage.anonimo")}</span></div>
                <div><span className="text-zinc-500">{t("soporte.mensajesPage.email")}</span> <span className="text-white">{mensajeSeleccionado.remitente_email || t("soporte.mensajesPage.sinEmail")}</span></div>
